...
使用 Python 标准库启动一个简单 HTTP 服务器，端口是 `9000`。
...
```powershell
curl.exe -i -x http://127.0.0.1:8080 http://demo-block.test/
```
...
```powershell
curl.exe -i -x http://127.0.0.1:8080 -X PATCH http://127.0.0.1:9000/
```
