...
批量测试使用独立目录保存数据，避免规则测试覆盖 Web 功能日志：
...
普通管理端启动后，首页“首页数据源”区域提供“当前运行 / 最近验收”切换，并提供三个证据详情入口：

- `Web 访问日志`：读取 `profile=web` 的访问、缓存、HTTPS 隧道等日志。
- `Web 拦截日志`：读取 `profile=web` 的域名、URL、方法、正文、认证和限流拦截日志。
- `当前规则变更`：读取代理服务器当前运行期间的规则增删改查和运行设置变更。
- `验收规则变更`：读取 `profile=rules` 的隔离批量验收证据，不覆盖手工操作记录。

规则变更详情页支持按操作类型和规则组独立查询，效果相当于对持久化 JSONL 审计记录执行只读筛选。

首页选择“最近验收”时，总请求、总拦截、各类拦截次数、域名排行、关键字排行、右侧访问/拦截日志和规则变更回显统一读取最近一次批量验收证据。选择“当前运行”时，上述区域统一读取当前代理进程数据。新增、删除、修改规则或保存运行设置时，首页会自动切回“当前运行”，避免把只读证据误认为实时配置。
...
两组规则均支持新增、删除、修改和替换整组。输入框支持单个 IPv4/IPv6 地址和 CIDR 网段，后端负责校验并标准化。统计区使用自适应网格，新增“客户端拦截”指标；点击该指标会进入日志详情页并自动查询 `client_ip_` 拦截原因。

客户端规则只限制代理端口 `8080`，不会限制管理端口 `8088`。因此现场演示中即使命中本机黑名单，也可以从管理前端删除规则恢复代理访问。
