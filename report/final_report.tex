...
本功能对应的请求解析、报文改写和上游转发核心代码见 \hyperref[app:forward]{【附录 A.1】}。
...
本功能对应的域名、客户端 IP/CIDR 和目标策略判定核心代码见 \hyperref[app:policy]{【附录 A.2】}。
...
对于可检查的响应，系统根据响应头声明的字符集解码正文，并兼容 UTF-8 与 GB18030；若响应使用 chunked 传输，系统先还原各数据块后再匹配，使跨块关键词也能被识别。正文统一转换为小写后与规则逐项比较。命中关键词时，原始网页正文不会发送给客户端，代理改为生成 \code{403 Forbidden} 提示页。提示页包含拦截原因、命中规则、请求方法、目标地址和客户端地址，因此浏览器中显示的内容能够直接说明过滤结果。

正文过滤仅适用于代理能够读取的明文 HTTP 响应。若网站使用 HTTPS，浏览器和网站之间的应用层数据经过 TLS 加密，普通 CONNECT 代理无法读取网页正文。

本功能对应的分块解析、字符集解码和正文规则匹配核心代码见 \hyperref[app:content]{【附录 A.3】}。
...
本功能对应的 CONNECT 双向转发核心代码见 \hyperref[app:connect]{【附录 A.4】}。

\subsection{缓存、认证与限流设计}

HTTP GET 缓存以“目标主机、端口和路径”组成缓存键，每个条目保存响应字节、状态码、创建时间和过期时间。请求到达时先检查条目是否存在且未过期；命中时仍按照当前正文规则重新检查缓存响应，未命中时连接上游。正文规则或缓存参数变化时系统清空旧缓存，避免规则热更新后继续返回旧网页。缓存容量达到上限时删除最早创建的条目。
...
缓存、代理认证和固定时间窗口限流的核心代码见 \hyperref[app:runtime]{【附录 A.5】}。
...
规则增删改查、缓存一致性处理和变更记录核心代码见 \hyperref[app:rules]{【附录 A.6】}。
...
日志写入、结构化查询与 CSV 导出的核心代码见 \hyperref[app:audit]{【附录 A.7】}。
...
\subsection{缓存、认证、限流与客户端地址策略验收}

缓存验收先启用缓存并清空旧条目，随后连续访问同一个 HTTP GET 地址。第一次日志应为 \code{CACHE_MISS} 与 \code{ALLOW}，第二次应为 \code{CACHE_HIT}。正文规则发生变化时，系统自动清空旧缓存，防止旧网页绕过新规则。
...
代理认证验收启用 Basic 认证。无凭据请求应返回 407，正确凭据请求能够继续访问，且 \code{Proxy-Authorization} 不转发给目标网站。

\screenshotfigure{manual-auth.png}{代理认证验收结果}{未认证请求的 407 响应和带正确代理账号请求的正常响应}
...
客户端 IP 黑白名单验收使用 \code{127.0.0.1} 或对应 CIDR。黑名单命中时返回 403，白名单非空且客户端未命中时同样拒绝访问。

\screenshotfigure{manual-client-ip.png}{客户端 IP 黑白名单验收结果}{客户端地址规则、403 结果以及 client\_ip\_blacklist 或 client\_ip\_not\_allowed 日志}
...
正文过滤故障排查发现，旧缓存曾在规则新增后直接返回原网页。修复后，正文规则与缓存参数变化会清空旧缓存，缓存命中也会按照当前正文规则重新检查；同时增加 chunked 正文还原和 GB18030 字符集测试。限流日志表明浏览器后台连接会共同消耗同一客户端 IP 的额度，因此现场演示其他模块前应关闭限流，单独验收限流时再启用并重置计数。
...
当前代理不解密 HTTPS 流量，因此无法检查 HTTPS 路径和正文。正文过滤已经支持 UTF-8、GB18030 和 chunked 文本响应；压缩响应与大文件流式过滤仍可继续完善。缓存和统计保存在进程内存中，重启后会清空；日志使用文件保存，在数据量较大时查询效率有限。
